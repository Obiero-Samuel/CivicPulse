// officer.js — officer portal logic
...
if (user && user.role !== 'authority_officer') {
	window.location.href = 'index.html';
}
...
async function loadAssignedReports() {
	...
	try {
		...
		el.innerHTML = data.reports.map(r => `
			<div class="report-item" onclick="openOfficerModal(${r.id}, '${esc(r.title).replace(/'/g, "&#39;")}')">
...
async function submitOfficerStatus(e) {
	...
	try {
		const res = await fetch(`${API_BASE}/reports/${officerModalReportId}/status`, {
			method: 'PATCH',
			headers: {
